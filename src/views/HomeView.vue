    ...
    <div class="wrap" ref="wrapEl">
        <header class="topbar">
            <div class="title">
                ...
                <span
                    class="sub"
                    style="color: white; font-size: 1.4rem; margin-left: 1rem"
                    >{{ nowText }}</span
                >
            </div>
            <div class="toolbar">
                <input
                    ...
                    @keydown.stop
                />
                <button class="btn ghost" @click="toggleFullscreen">
                    전체 화면
                </button>
        ...
        <main class="main">
            ...
            <aside class="aside aside-summary">
                <div class="counter">
                    <div class="card">
                        <span class="label" style="font-size: 17px">총원</span
                        ><span class="pill gray" style="font-size: 17px"
                            >{{ TOTAL }}명</span
                        >
                    </div>
                    <div class="card">
                        <span class="label" style="font-size: 17px"
                            >현원(교실)</span
                        ><span class="pill green" style="font-size: 17px"
                            >{{ present }}명</span
                        >
                    </div>
                    <div class="card">
                        <span class="label" style="font-size: 17px">결원</span
                        ><span class="pill red" style="font-size: 17px"
                            >{{ absent }}명</span
                        >
                    </div>
                </div>
                    ...
                    >
                        <div class="lane-header">
                            ...
                            <div class="lane-count">
                                <span>{{ lanes.room.length }}명</span>
                            </div>
                        </div>
                        <div class="grid">
                            <button
                                v-for="n in sorted(lanes.room)"
                                :key="`room-${n}`"
                                class="chip"
                        ...
                        >
                            <div class="lane-header">
                                <div class="lane-title">
                                    <span>{{ laneTitles[lane] }}</span>
                                </div>
                                <div class="lane-count">
                                    <span>{{ lanes[lane].length }}명</span>
                                </div>
                            </div>
                            <div class="stack">
                                <!-- 일반 레인: 기존 칩 버튼 렌더 -->
...
const loadState = (): BoardState => {
    try {
        ...
        const legacyOut = (parsed as any).out as number[] | undefined;
        if (Array.isArray(legacyOut)) {
            (parsed.etc ??= []).push(...legacyOut);
        }
        // 무결성 보정: 누락된 번호는 교실로
        const seen = new Set<number>();
        LANES.forEach((l) => (parsed[l] ??= []).forEach((v) => seen.add(v!)));
        for (let i = 1; i <= TOTAL; i++)
            if (!seen.has(i)) (parsed.room as number[]).push(i);
...
onMounted(async () => {
    ...
    try {
        const response = await axios.get(
            "https://back-dimiboard.coder.ac/notice",
        );
...
// 모두 교실 / 초기화
function allIn() {
    const all = Array.from(new Set(LANES.flatMap((l) => lanes[l]))).sort(
        (a, b) => a - b,
    );
...
function destroyGhost() {
    if (dragGhost.value?.parentElement)
...
:root {
    ...
    --accent: #5ac8fa;
    --shadow:
        0 10px 24px rgba(0, 0, 0, 0.35), inset 0 1px 0 rgba(255, 255, 255, 0.02);
    --radius: 16px;
    --chip: #ced1de3b;
    --chip-hover: #ced1de61;
...
.lane {
    background: linear-gradient(180deg, #151926 0%, #121623 100%);
...
.chip.highlight {
    ...
    outline-offset: 2px;
    filter: drop-shadow(0 0 10px rgba(90, 200, 250, 0.5));
}
